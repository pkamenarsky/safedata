...
-- | Derive an instance of 'SafeData'.
--
...
--   @
--data T0 b = T0 b Int
--deriveSafeData 1 'base ''T0
--   @
--
--   we generate
--
--   @
--instance (SafeData a, SafeData b) =>
--         SafeData (T0 b) where
...
--deriveSafeData 2 'extension ''T
--
--instance SafeData b => Migrate (T a b) where
...
--   'deriveSafeDataSimple', but never more (as we don't call
--   'getSafePut'/'getSafeGet' for types that aren't needed).
--
--   Note that you may use 'deriveSafeDataSimple' with one
--   version of your data type and 'deriveSafeData' in another
--   version without any problems.
deriveSafeData :: Version a -> Name -> Name -> Q [Dec]
deriveSafeData = internalDeriveSafeData Normal

deriveSafeDataAll :: Name -> Q [Dec]
deriveSafeDataAll = internalDeriveSafeDataAll Normal

deriveSafeDataIndexedType :: Version a -> Name -> Name -> [Name] -> Q [Dec]
deriveSafeDataIndexedType = internalDeriveSafeDataIndexedType Normal

-- | Derive an instance of 'SafeData'.  The instance derived by
--   this function is simpler than the one derived by
--   'deriveSafeData' in that we always use 'safePut' and
...
--   the past.
--
--   Note that you may use 'deriveSafeData' with one version of
--   your data type and 'deriveSafeDataSimple' in another version
--   without any problems.
deriveSafeDataSimple :: Version a -> Name -> Name -> Q [Dec]
deriveSafeDataSimple = internalDeriveSafeData Simple

deriveSafeDataSimpleIndexedType :: Version a -> Name -> Name -> [Name] -> Q [Dec]
deriveSafeDataSimpleIndexedType = internalDeriveSafeDataIndexedType Simple
...
--   'deriveSafeDataSimple'), but we also always write a 'Word8'
...
--   'deriveSafeData' or 'deriveSafeDataSimple', but may use more
...
--
--   Note that you may use 'deriveSafeData' with one version of
--   your data type and 'deriveSafeDataHappstackData' in another version
--   without any problems.
deriveSafeDataHappstackData :: Version a -> Name -> Name -> Q [Dec]
deriveSafeDataHappstackData = internalDeriveSafeData HappstackData

deriveSafeDataHappstackDataIndexedType :: Version a -> Name -> Name -> [Name] -> Q [Dec]
deriveSafeDataHappstackDataIndexedType = internalDeriveSafeDataIndexedType HappstackData
...
internalDeriveSafeDataAll :: DeriveType -> Name -> Q [Dec]
internalDeriveSafeDataAll deriveType tyName = do
  mfs <- followMigrationChain $ ConT tyName
  concat <$> sequence
    [ internalDeriveSafeData deriveType (Version v) b t
    | (v, b, ConT t) <- zip3 [0..] ('base:repeat 'extension) (reverse mfs)
    ]

internalDeriveSafeData :: DeriveType -> Version a -> Name -> Name -> Q [Dec]
internalDeriveSafeData deriveType versionId kindName tyName = do
  info <- reify tyName
  case info of
...
internalDeriveSafeDataIndexedType :: DeriveType -> Version a -> Name -> Name -> [Name] -> Q [Dec]
internalDeriveSafeDataIndexedType deriveType versionId kindName tyName tyIndex' = do
...
conSize InfixC{}             = 2
conSize ForallC{}            = error "Found complex constructor. Cannot derive SafeData for it."
...
conNames ForallC{}            = error "Found complex constructor. Cannot derive SafeData for it."
